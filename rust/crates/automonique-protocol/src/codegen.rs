...
export interface MutationApproval {
  ...
  readonly preview: MutationPreviewRef; readonly request_digest: WorkContextRequestDigest;
}
...
export interface MutationReceipt {
  ...
  readonly outcome: ReceiptOutcome; readonly preview: MutationPreviewRef; readonly recorded_at_ms: bigint;
...
const PREVIEW_FIELDS = ["approval", "current", "effective_authority", "expires_at_ms", "inherited_authority", "issued_at_ms", "preview", "proposal", "resulting", "schema"] as const;
const APPROVAL_FIELDS = ["decided_at_ms", "decided_by", "decision", "expires_at_ms", "id", "idempotency_key", "preview", "request_digest"] as const;
const SUBMISSION_FIELDS = ["approval", "idempotency_key", "preview", "request_digest", "schema", "submitted_at_ms"] as const;
const RECEIPT_FIELDS = ["approval_id", "id", "idempotency_key", "outcome", "preview", "recorded_at_ms", "request_digest", "resulting_revision", "schema"] as const;
...
export function validateMutationPreview(value: MutationPreview): MutationPreview {
  ...
  const proposal=validateWorkContextMutationProposal(value.proposal); const current=value.current===null?null:validateWorkContextRecord(value.current); const resulting=validateWorkContextRecord(value.resulting); const inherited=validateAuthority(value.inherited_authority); const effective=validateAuthority(value.effective_authority); const requested=requestedAuthority(proposal.intent);
  if(value.approval!=="not_required"&&value.approval!=="required") workContextRefusal("approval requirement is invalid");
  if(value.issued_at_ms<0n||value.expires_at_ms<=value.issued_at_ms) workContextRefusal("preview expiry is invalid");
