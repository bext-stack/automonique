...
//!
//! Storage is process memory on purpose, bounded by the byte ceiling plus
//! per-object metadata; the charging policy is stated in
//! `docs/operations/temporary-storage-budget.md`. The memory is the
//! supervisor's, not the run's cgroup.
//!
...
//!
//! The one addition over a plain quota filesystem is the **exceedance
//! latch**: the first refusal the ledger records is published through a shared
//! [`ExceedanceChannel`] the moment it happens, so the supervisor can end a
//! run on its first `ENOSPC`/`EDQUOT` without polling `statfs`.

use crate::tempfs_ledger::{
    Exceedance, Ledger, LedgerSnapshot, MAX_NAME_BYTES, STATFS_BLOCK_BYTES, TemporaryStorageBudget,
};
use fuser::{
...
///
/// A lock-free latch: `sequence` is zero until the first refusal, then holds
/// a monotonically increasing count, and `first` holds the refusal that set
/// it. The supervisor reads [`Self::first`] on its own poll interval and ends
/// the run the first time it is `Some`, without a `statfs` round trip.
#[derive(Debug, Default)]
pub struct ExceedanceChannel {
    count: AtomicU64,
    resource: AtomicU64,
    requested: AtomicU64,
    used: AtomicU64,
...
const RESOURCE_UNSET: u64 = 0;
const RESOURCE_BYTES: u64 = 1;
const RESOURCE_OBJECTS: u64 = 2;

impl ExceedanceChannel {
    /// Record `exceedance` as the first one, if none has been recorded yet.
    ///
    /// The count grows on every refusal; the detail fields are written only
    /// once, on the transition from zero, so the supervisor always reads the
    /// *first* refusal even if several arrive before it polls.
    fn record(&self, exceedance: Exceedance) {
        let previous = self.count.fetch_add(1, Ordering::AcqRel);
        if previous == 0 {
            let resource = match exceedance.resource {
                crate::tempfs_ledger::Resource::Bytes => RESOURCE_BYTES,
                crate::tempfs_ledger::Resource::Objects => RESOURCE_OBJECTS,
            };
            self.requested
                .store(exceedance.requested, Ordering::Relaxed);
            self.used.store(exceedance.used, Ordering::Relaxed);
            self.ceiling.store(exceedance.ceiling, Ordering::Relaxed);
            // The resource is stored last and with release ordering, so a
            // reader that sees a non-zero resource sees the fields beside it.
            self.resource.store(resource, Ordering::Release);
        }
    }

    /// How many refusals the ledger has recorded.
    #[must_use]
    pub fn count(&self) -> u64 {
        self.count.load(Ordering::Acquire)
    }

    /// The first refusal, once one has been recorded.
    #[must_use]
    pub fn first(&self) -> Option<Exceedance> {
        let resource = match self.resource.load(Ordering::Acquire) {
            RESOURCE_BYTES => crate::tempfs_ledger::Resource::Bytes,
            RESOURCE_OBJECTS => crate::tempfs_ledger::Resource::Objects,
            _ => return None,
        };
        Some(Exceedance {
            resource,
    ...
    /// Whether any refusal has been recorded.
    #[must_use]
    pub fn exceeded(&self) -> bool {
        self.resource.load(Ordering::Acquire) != RESOURCE_UNSET
    }
}
...
impl Node {
    ...
    fn attr(&self, ino: u64) -> FileAttr {
        ...
        FileAttr {
            ...
            gid: self.gid,
            rdev: 0,
            blksize: u32::try_from(STATFS_BLOCK_BYTES).expect("block size fits"),
            flags: 0,
        }
    }
}

/// The tree, the ledger, and the exceedance channel, behind one lock.
pub struct State {
...
impl State {
    ...
    fn reserve_bytes(&mut self, delta: u64) -> Result<(), Errno> {
        self.ledger.reserve_bytes(delta).map_err(|exceedance| {
            self.channel.record(exceedance);
            refused(exceedance)
        })
    }

    fn reserve_object(&mut self) -> Result<(), Errno> {
        self.ledger.reserve_object().map_err(|exceedance| {
            self.channel.record(exceedance);
            refused(exceedance)
        })
    }
    ...
    fn create_object(
        ...
        if bytes > 0
            && let Err(errno) = self.reserve_bytes(bytes)
        {
    ...
    fn resize(&mut self, ino: u64, new_size: u64) -> Result<(), Errno> {
        ...
        if new_size > current {
            self.reserve_bytes(new_size - current)?;
        } else {
    ...
    fn write_at(&mut self, ino: u64, offset: u64, data: &[u8]) -> Result<u32, Errno> {
        ...
        if end > current {
            self.reserve_bytes(end - current)?;
        }
        let node = self.node_mut(ino)?;
...
impl QuotaFs {
    /// An empty tree under `budget`, owned by `uid`/`gid`, publishing its
    /// first refusal through `channel`.
    #[must_use]
    pub fn new(
        budget: TemporaryStorageBudget,
        uid: u32,
        ...
        Self {
            state: Arc::new(Mutex::new(State::new(budget, uid, gid, channel))),
        }
    }
...
impl Filesystem for QuotaFs {
    ...
    fn link(
        ...
        // ledger charges them once; accounting stays one-to-one.
        reply.error(Errno::EPERM);
    }
...
mod tests {
    ...
    #[test]
    fn the_channel_publishes_the_first_refusal_and_counts_the_rest() {
        let channel = ExceedanceChannel::default();
        assert!(!channel.exceeded());
        assert_eq!(channel.first(), None);
        let first = Exceedance {
            resource: crate::tempfs_ledger::Resource::Bytes,
            requested: 4096,
            used: 1_048_576,
            ceiling: 1_048_576,
        };
        channel.record(first);
        // A later refusal does not overwrite the first, but does count.
        channel.record(Exceedance {
            resource: crate::tempfs_ledger::Resource::Objects,
            requested: 1,
            used: 16,
            ceiling: 16,
        });
        assert!(channel.exceeded());
        assert_eq!(channel.first(), Some(first));
