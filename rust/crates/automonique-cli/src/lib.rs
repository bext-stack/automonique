    ...
    E: Write,
{
    ...
    let command = match (command.as_deref(), first, second, third) {
        ...
        (Some(command), None, None, None) if command == "rollback" => Command::Rollback,
        ...
        (Some(command), Some(action), second, third) if command == "runs" => {
            ...
            match (action.to_str(), flags.as_slice()) {
                ...
                // The spool path is named rather than derived. This CLI resolves
                // no ambient state directory for anything else either — see
                // `attempt`, which takes its socket path the same way — and a
                // guessed one would read whichever daemon's tree happened to be
                // under the default.
                (Some("tail"), [spool_root, run_id]) => Command::Runs(runs::Operation::Tail {
                    spool_root: spool_root.clone(),
                    run_id: run_id.clone(),
                    cursor: None,
                }),
                (Some("tail"), [spool_root, run_id, cursor]) => {
                    Command::Runs(runs::Operation::Tail {
                        spool_root: spool_root.clone(),
                        run_id: run_id.clone(),
        ...
        (Some(command), Some(operation), Some(socket_path), Some(run_id))
            if command == "progress" =>
        {
            let cursor = arguments.next();
            let extra = arguments.next();
            match (operation.to_str(), extra) {
                (Some("subscribe"), None) => Command::Progress(progress::Operation::Subscribe {
                    socket_path,
                    run_id,
                    cursor,
                }),
    ...
    let json = match command {
        ...
        Command::Rollback => {
            return admin_rollback(runtime.as_deref(), &mut stdout, &mut stderr);
        }
        Command::ReloadStatus { reload_id } => {
