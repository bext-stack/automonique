...
export function encodeResourceListingPage(page: ResourceListingPage): Uint8Array {
  ...
  // Held to the decoder rather than to a second copy of its rules. An encoder
  // that could emit a page the decoder refuses would make a fixture that
  // proves nothing, and a second copy of the page invariants is exactly the
  // drift this contract keeps paying for.
  resourceListingPage(parseCanonical(bytes), bytes);
  return bytes;
}
export function encodeResourceListingResync(resync: ResourceListingResync): Uint8Array {
  ...
  resourceListingResync(parseCanonical(bytes));
  return bytes;
}
