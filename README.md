...
| Brokered egress | `egress-destinations` | Starts one loopback CONNECT broker per run on a kernel-assigned ephemeral port, allowing exactly the host/port pairs the file names and denying everything else. Absent, every brokered document is refused. Removing a line the configured provider dials takes every run down and says nothing: [the destination policy is the run engine's lifeline](docs/operations/egress-destinations.md). |
