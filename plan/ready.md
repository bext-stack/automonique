# Ready set

GENERATED by `plan/check.py` — do not edit by hand.

An item is ready when every dependency is `done`. Selecting work means taking a ready ID, reading its contract, and refusing it if any gate it depends on is still open.

- items total: **375**
- done: **11**
- ready now: **32**
- blocked: **332**

## Focus ledger

Product work is what ships. Discovery feeds the specification. Harness work builds the machine that develops the product and is frozen by [`GATE-HARNESS`](gates.md#gate-harness). If the harness column starts leading again, that is the regression this table exists to show.

| Class | Items | Done | Contracts | Ready |
|---|---|---|---|---|
| product | 330 | 2 | 25 | 23 |
| discovery | 23 | 9 | 18 | 9 |
| harness | 22 | 0 | 5 | 0 |

## Selectable now

| ID | Epic | Title | Licence | Contract |
|---|---|---|---|---|
| `BOOT-002` | BOOT | Optional integration identity hardening | Elastic-2.0 | [contract](contracts/BOOT-002.md) |
| `BOOT-003` | BOOT | Pre-publication scrub gate | Elastic-2.0 | [contract](contracts/BOOT-003.md) |
| `BOOT-004` | BOOT | Parity-oracle boundary | Elastic-2.0 | [contract](contracts/BOOT-004.md) |
| `R0-01` | R0 | Current contract inventory | Elastic-2.0 | [contract](contracts/R0-01.md) |
| `R0-08` | R0 | Machine-readable parity ledger | Elastic-2.0 | [contract](contracts/R0-08.md) |
| `R0-09` | R0 | Identity/data/operations inventory | Elastic-2.0 | [contract](contracts/R0-09.md) |
| `R0-10` | R0 | Baseline recovery drill | Elastic-2.0 | [contract](contracts/R0-10.md) |
| `R0-11` | R0 | Shell decision fixture | Elastic-2.0 | [contract](contracts/R0-11.md) |
| `R0-13` | R0 | Automonique identifier inventory | Elastic-2.0 | [contract](contracts/R0-13.md) |
| `R1-03` | R1 | Length-delimited local protocol codec | Elastic-2.0 | [contract](contracts/R1-03.md) |
| `R1-04` | R1 | Unix peer authorization | Elastic-2.0 | [contract](contracts/R1-04.md) |
| `R1-05` | R1 | Release manifest | Elastic-2.0 | [contract](contracts/R1-05.md) |
| `R1-06` | R1 | Cross-language fixtures | Elastic-2.0 | [contract](contracts/R1-06.md) |
| `R1-07` | R1 | `automonique doctor` | Elastic-2.0 | [contract](contracts/R1-07.md) |
| `R1-08` | R1 | Provider capability contract | Elastic-2.0 | [contract](contracts/R1-08.md) |
| `R1-09` | R1 | Provider event contract | Elastic-2.0 | [contract](contracts/R1-09.md) |
| `R1-10` | R1 | Upstream schema pipeline | Elastic-2.0 | [contract](contracts/R1-10.md) |
| `R1-11` | R1 | TypeScript codegen spike | Elastic-2.0 | [contract](contracts/R1-11.md) |
| `R1-12` | R1 | Domain event/action contracts | Elastic-2.0 | [contract](contracts/R1-12.md) |
| `R1-13` | R1 | Identity and policy contracts | Elastic-2.0 | [contract](contracts/R1-13.md) |
| `R1-14` | R1 | Workspace/artifact contracts | Elastic-2.0 | [contract](contracts/R1-14.md) |
| `R1-15` | R1 | Execution-host lifecycle contract | Elastic-2.0 | [contract](contracts/R1-15.md) |
| `R1-16` | R1 | External connector contract | Elastic-2.0 | [contract](contracts/R1-16.md) |
| `R1-17` | R1 | Canonical/legacy compatibility contract | Elastic-2.0 | [contract](contracts/R1-17.md) |
| `R1-18` | R1 | Sandbox contracts | Elastic-2.0 | [contract](contracts/R1-18.md) |
| `R1-19` | R1 | Canonical Rust namespace gate | Elastic-2.0 | [contract](contracts/R1-19.md) |
| `R1-20` | R1 | Context and learning contracts | Elastic-2.0 | [contract](contracts/R1-20.md) |
| `R1-21` | R1 | Tool and extension contracts | Elastic-2.0 | [contract](contracts/R1-21.md) |
| `R1-22` | R1 | Automation and goal contracts | Elastic-2.0 | [contract](contracts/R1-22.md) |
| `R1-23` | R1 | Public protocol contracts | Elastic-2.0 | [contract](contracts/R1-23.md) |
| `R1-24` | R1 | Model/media/executor contracts | Elastic-2.0 | [contract](contracts/R1-24.md) |
| `R1-25` | R1 | Cross-surface interaction contracts | Elastic-2.0 | [contract](contracts/R1-25.md) |

## Next to unblock

The first blocked items behind the current ready set:

- `R0-02` Sanitized fixture corpus — waits on `R0-01`, gate `GATE-ORACLE`
- `R0-07` Provider transcript corpus — waits on gate `GATE-ORACLE`
- `R0-12` Channel connector corpus — waits on contract
- `R0-14` Sandbox host-capability spike — waits on contract
- `R0-15` Provider/tool egress spike — waits on contract
- `R0-19` Minimal `automonique-lab` — waits on gate `GATE-HARNESS`
- `R0-20` Role-policy trials — waits on `R0-21`, gate `GATE-HARNESS`
- `R0-21` Commit metrics and baselines — waits on `R0-19`, gate `GATE-HARNESS`
- `R0-22` Harness reload and merge train — waits on `R0-20`, `R0-21`, gate `GATE-HARNESS`
- `R0-23` Self-hosting levels/policy — waits on gate `GATE-HARNESS`

## Unblocked but unspecified

Dependency- and gate-clear, but no contract exists, so they are not selectable. Writing one of these contracts is itself useful work and lowers `contracts_missing`. **Ordered product first** — specifying product work is what makes product work selectable, and nothing else in this repository does that.

  `R0-12`, `R0-14`, `R0-15`

## Gate-blocked work

Items that a gate holds back independently of their dependencies:

| Gate | Items |
|---|---|
| [`GATE-HARNESS`](gates.md#gate-harness) | `R0-19`, `R0-20`, `R0-21`, `R0-22`, `R0-23`, `R0-24` …and 16 more |
| [`GATE-ORACLE`](gates.md#gate-oracle) | `R0-02`, `R0-07` |
| [`GATE-SCRUB`](gates.md#gate-scrub) | `R10-01`, `R10-02`, `R10-03`, `R10-04`, `R10-05`, `R10-06` …and 15 more |
