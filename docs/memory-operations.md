...
```sh
automonique-memory backfill-telegram /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>' /private/state/automonique.sqlite3 <bot-id>
```
...
```sh
automonique-memory link-identity /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>' slack automonique-slack '<immutable-team-id>' '<immutable-user-id>'
```
...
Jean's recent chat store can be imported selectively after that identity check. Only the exact Telegram identity, legacy Slack scopes ending in the exact Slack user ID, and Slack threads whose durable event author matches that ID are admitted. Slash commands, unrelated actors, and rows older than 90 days are skipped; reruns are idempotent:

```sh
automonique-memory backfill-jean /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>' /private/jean.db '<telegram-user-id>' '<slack-user-id>'
```
...
```sh
automonique-memory export-obsidian /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>' /private/monique-memory
```

After editing a generated `Memories/M-….md` note, import the edit as a proposal:

```sh
automonique-memory propose-obsidian /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>' /private/monique-memory/Memories/M-….md
```
...
```sh
automonique-memory hygiene /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>'
```

An owner-reviewed preference may be seeded operationally without creating an unreviewed proposal:

```sh
automonique-memory remember-active /private/state/agent-memory.sqlite3 inklura 'telegram:<immutable-user-id>' 'Reviewed preference text'
```
