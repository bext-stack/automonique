...
| SH3 self-reloading | A candidate development control plane reloads while sessions, workers, builds and evidence survive or reconcile | stable launcher and dev leases |
| SH4 self-verifying | Stable, candidate and an independent builder evaluate the same revision; candidate self-rebuild and recovery pass | stable verifier plus independent builder |
...
The stable development generation and candidate use different databases, sockets, runtime directories, service identities, credential audiences, transport installations, workspace registries, artifact namespaces, leases and outboxes. Candidate services are named by immutable source/build digest and cannot bind a stable or production endpoint.

The tiny stable launcher/bootstrap verifier is upgraded less frequently than the product. It accepts only signed manifests, immutable paths, expected digests and typed lifecycle operations. Changing that verifier is possible, but the old verifier must build/verify its replacement and remain available for rollback.
...
tools/automonique-lab/            Rust durable development control plane and brokers
```
...
- minimum kernel/systemd/sandbox capabilities for tests;
...
## Self-build and independent rebuild
...
3. An independent builder B builds R from a clean source acquisition in declared environment E, producing A3 and provenance P3.
4. The verifier compares A1/A2/A3, manifests, dependency resolution, generated source and test evidence.
...
Protected `main`, stable release tags, public package publication, signing keys and production deployment remain outside candidate credentials at every ceiling. Required-status policies include independent build/review identities so a candidate cannot satisfy every gate with one compromised context.
...
- Candidate code never runs inside the stable verifier/launcher process.
...
- Implementer, reviewers, fixer, independent builder and promoter have distinct recorded roles; policy states which identities may coincide for low-risk development.
...
- A candidate may request rollback; stable or the launcher performs it.
...
automoniquectl self-host status|doctor|build|test|launch|compare|reload|rollback
automoniquectl self-host promotion prepare|inspect|approve
```
...
An independent builder produces authenticated provenance; reproducible or declared normalized comparisons pass with no unexplained mismatch.

### Gate 6 — development integration

Policy-controlled integration into a bot-owned branch survives conflicts, current CI and external review. Candidate cannot modify required checks or protected branches.
