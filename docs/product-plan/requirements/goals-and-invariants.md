...
2. Preserve Slack, Telegram, dashboard, TUI, Fleet, and agent-run continuity.
...
6. Keep operator commands simple: `automonique reload`, `automonique rollback`, `automonique status`, and `automonique doctor`, with tested `legacyctl` aliases during migration.
...
16. Implement the program through a durable, measurable AI development loop whose configured reviews, resource bounds and commit evidence raise confidence without granting autonomous merge or production authority.
...
- A job is a separate supervised unit whose lifetime is independent of the `automonique` daemon.
...
- Slack posts/reactions, GitHub comments, Support mail, Fleet reports, and privileged actions use stable idempotency keys where the remote API permits them.
...
- The web dashboard, the `automonique` CLI, and `automonique tui` (plus supported legacy aliases) use the same typed server-side action contracts and durable authority.
...
- Review count is owner-configurable and recorded truthfully; a configured
  blocking finding requires new evidence, a corrected candidate or an explicit
  owner disposition.
...
- Stable and candidate generations use distinct state, sockets, process identities, credentials, workspaces, artifacts, leases and outboxes; a candidate cannot bind a stable/production endpoint.
...
- Candidate-authored tests, metrics, logs and provenance are claims until
  observed by stable or accepted by the owner-configured control plane.
- A candidate cannot write its own `owner_verified`, `promotable` or `promoted`
  transition, weaken its required checks or access
  protected-branch/release/deployment authority.
...
- A clean rebuild, when configured, has no unexplained output, dependency,
  generated-source or behavior mismatch before promotion eligibility.
- Recursive improvement is bounded by approved objectives,
  depth/concurrency/time/token/cost limits and explicit owner decisions for
  product, security, legal, metrics, privilege and production changes.
...
- Missing enforcement required by the selected execution backend, attestation drift or an orphan boundary fails closed and quarantines affected provider work.
