...
- every machine-readable parity-ledger entry, including Inklura tenant fencing, client publication, companions, learned targets, reconciliation/audit, notifications, operational commands and ignored-message behavior.
...
- while sending Slack/GitHub/Manage/Support effects;
...
- Slack/Telegram/Manage timeouts and duplicate deliveries;
...
- Validate mount/user/PID/network namespace and systemd hardening behavior on the production kernel; required features fail closed rather than degrading silently.
...
- Run `automoniquectl audit --preview` (and the supported `legacyctl` alias), reject a stale plan, apply an exact plan revision and prove a second preview proposes zero changes.
...
- Run representative mechanical, durable-state and provider/transport trials with one implementer, at least two fresh-context adversarial reviewers and a separate fixer; reject self-approval and unresolved blockers.
...
- Kill/corrupt C0, C1, candidate database, build broker, provider and independent builder at every boundary; stable remains healthy and restores/abandons the candidate without production impact.
...
- bootstrap stage/manifest/toolchain health, source/build fingerprints, candidate state/gate age, self-host reload/rebuild status, independent provenance and reproducibility mismatches.
...
The SH0 seed and `automonique-bootstrap` graduate here. Self-host candidates remain in fixture/replay mode until namespace and credential isolation passes; independent SH4 rebuild evidence is required before the candidate may alter bootstrap, sandbox, authorization or promotion code.
...
Rust becomes the default service. Keep the previous compatible release, database backup procedure, and tested rollback for a defined soak window.
