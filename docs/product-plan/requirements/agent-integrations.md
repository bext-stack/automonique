...
Start one `codex app-server --listen stdio:// --strict-config` inside an execution-host unit. Use stdio JSONL, not remote WebSocket. The installed CLI can generate a JSON Schema bundle for its exact version; check that bundle into adapter fixtures and include its hash in the host manifest.
...
Start a dedicated `opencode serve` instance inside an execution-host unit, bound to loopback on an allocated port. Set a random high-entropy Basic Auth password through a protected credential descriptor/environment mechanism, never argv or a durable spec. Generate or vendor a Rust client from the server's OpenAPI 3.1 document for the pinned opencode version.
