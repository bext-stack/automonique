...
`automoniquectl reload <release>` (or the `legacyctl` compatibility alias) sends the immutable release path and expected manifest hash through the systemd/launcher-owned stable admin socket. Generation N validates opened immutable files without loading credentials into the CLI.
...
N starts the target `automoniqued` with:

- generation ID N+1;
- reload epoch E;
- database path;
- accepted admin descriptors plus a one-time readiness/handoff socket; the candidate never unlinks or rebinds the stable admin socket;
- systemd notification descriptor/environment;
- release manifest path.

Secrets are read by N+1 from the same protected service environment or credential mechanism, never sent over the handoff payload.
...
- Manage and Support configuration validation without mutation.
...
- adopts every active attempt- or session-scoped execution host by unit and control socket;
...
Only then is systemd `MAINPID`/`READY=1` transferred to N+1.
...
| N+1 fails after transfer but before active proof | leases return to N if alive; otherwise restart manager activates last compatible generation |
...
| Runner unavailable during adoption | verify systemd unit and status file; mark lost only after both prove terminal/unreachable |
...
5. Enumerate active transient execution-host units and host/attempt manifests.
...
A candidate's success report is not independent verification. Stable observation and the separately authenticated rebuild remain required before `promotable`. The complete state machine is in [Self-hosting and bootstrap](self-hosting-and-bootstrap.md).
...
```text
automoniquectl reload [release] [--wait]
automoniquectl rollback [--wait]
automoniquectl generations
automoniquectl reload-status <reload-id>
automoniquectl doctor --reload
automoniquectl runs
automoniquectl attach <run-id>
automoniquectl tui
automonique-tui [--attach <run-or-session-id>] [--workspace <name>]
```

The legacy `legacyctl` and `legacy-tui` commands forward to the same socket and implementation during the compatibility window. `automoniquectl doctor --reload` runs all non-mutating compatibility/readiness checks without starting a candidate, including the published TypeScript SDK compatibility range and schema digest.
