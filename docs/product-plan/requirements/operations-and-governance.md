...
Invariant checkers compare database state, systemd units, runner manifests, provider sessions, artifacts, GitHub truth, Slack/Telegram presentation, Fleet state and outboxes. Repair always follows preview → immutable plan → revalidation → apply → verify.
