...
- required kernel/execution-backend features and accepted enforcement implementation digests;
...
The host records an `EnforcementAttestation` containing the resolved real paths, namespaces, process-group/cgroup/backend IDs, kernel/boot identity, effective optional-supervisor properties, Landlock ABI/ruleset digest, seccomp digest, egress-policy digest, credential delivery mode and any verified external-daemon context.
...
Each attempt- or session-scoped execution host runs in a dedicated execution-backend process boundary. Apply and attest, as supported by the selected implementation:
...
- Prefer sealed inherited descriptors or an equivalent one-time protected mechanism over ordinary environment variables; supervisor credential stores are optional adapters.
...
- Each third-party TypeScript provider, MCP server or companion runs in its own child process boundary/sandbox with a declared digest, capabilities, credentials, paths, egress and resource budget.
...
Control-plane generation handoff adopts the existing host and revalidates its attestation digest, execution-backend and namespace identity. Reload never rebuilds the sandbox around a live provider. A missing/mismatched enforcement object quarantines the host and permits only bounded observation, reconciliation or cancellation.

## Self-hosting candidate boundary

Development self-hosting adds a stable/candidate boundary above ordinary worker isolation:

- candidate processes use digest-named runtime identities, UID/runtime paths, sockets, network namespace, database, artifact store and credential audience;
...
- clean rebuild workers, when configured, record their environment and
  provenance truthfully; a distinct workload identity is optional.

The stable bootstrap verifier/lifecycle owner never loads candidate libraries or executes candidate hooks in-process. Candidate escape, credential discovery, stable-socket binding, evidence tampering and provenance forgery are explicit negative-test families.
...
- required profiles fail closed on every missing kernel or selected-backend feature;
...
- the privileged sandbox launcher/broker parsers pass fuzzing and the
  owner-configured security review policy;
