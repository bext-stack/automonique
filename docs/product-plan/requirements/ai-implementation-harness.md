...
```text
crates/automonique-lab/         orchestrator library, reached via `automonique lab`
...
- provider/model selection and recorded implementer/reviewer contexts when the
  work unit requests separate review;
...
select ready work from the dependency DAG
  ...
  -> run the work unit's configured review passes (zero or more)
...
Review policy is explicit per work unit and may require zero, one or multiple
fresh contexts. When a separate review is requested, reviewers receive the
original contract, source baseline, frozen diff and test evidence but not the
implementer's persuasive narrative. Review records never claim independence
when the same context or identity performed both roles. Provider diversity and
role separation are optional hardening for high-risk changes, selected by the
owner rather than universal prerequisites.
...
3. Trial three representative units with the owner-configured role policy: one
   mechanical port, one durable-state transition and one provider/transport
   boundary.
...
- all acceptance gates pass and configured reviews have no unresolved blockers;
...
- truthful role records and deterministic finding resolution when review is configured;
...
- the first immutable SH0 seed builds an isolated candidate and hands its
  lifecycle into reproducibility and owner-configured promotion gates rather
  than treating a successful local build as trust.
