...
3. It owns provider connection/session/turn state independently of `automoniqued`.
...
- Codex App Server and opencode servers live inside session-scoped host units so an Automonique generation reload does not disconnect them.
...
- a [feature-parity ledger](feature-parity.md) for Support/Inklura, GitHub/client publication, companions, learned targets, live feeds, notifications, operational commands, reconciliation and the isolated shell subsystem;
...
- [client experiences](../requirements/client-experience-and-surfaces.md), including the Rust TUI, SDK-backed web app, Tauri desktop, mobile/PWA access, setup/update/import and constrained UI customization;
...
The breadth of the program makes manual prompt-by-prompt execution an avoidable risk. The plan now begins with [Automonique's AI implementation harness](../requirements/ai-implementation-harness.md): a separate durable work DAG, isolated path/worktree leases, central build scheduling, one bounded implementer, at least two independent adversarial reviewers, a fixer and serialized integration. Compiler/test/parity/security/performance failures become explicit queues, while incomplete work is resumed across agent or harness reload instead of being declared done from prose.
...
## First-run launcher review

Self-hosting still had a practical chicken-and-egg gap: the documented Rust bootstrap/lab could not be the program that initially built itself. [Initial development launcher](initial-development-launcher.md) adds a deliberately temporary stage-minus-one layer: a small Bash entry point and finite Bun coordinator using a checked seed DAG. One `start` command probes the checkout/toolchains/providers, prints an exact effect/budget plan, requires its digest, launches a persistent user unit and hands development ownership once to SH0.

The temporary layer cannot extend its DAG or publish remotely. It defaults to one worker, isolates provider adapters, denies agent Git mutation, survives detach/reboot from XDG state and retires after the Rust lab imports its evidence. This keeps the first command usable without allowing bootstrap convenience code to become an unreviewed permanent control plane.
