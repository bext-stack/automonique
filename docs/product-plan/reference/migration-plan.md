...
- Complete the machine-readable [feature-parity ledger](feature-parity.md), including client-portal tenant fencing, internal/client publication, companions, learned targets, audits, notifications, operational commands, deploy webhook and ignored-message behavior.
...
- Trial one mechanical port, one durable-state unit and one provider/transport
  unit under the owner-configured review policy; correct the loop and rerun from
  identical bases.
...
- Export sanitized fixtures for Slack, Telegram, Fleet, Support, and all four agent backends.
...
- Prototype the portable foreground generation lifecycle:
  - explicit readiness and handoff messages;
  - old/new process overlap and fenced ownership;
  - failed candidate recovery and bounded drain;
  - attempt- and session-host reconnect plus descendant cleanup;
  - capability mapping for optional systemd, launchd/container, and direct-process adapters.
- Pin the foreground baseline and record evidence that would justify recommending a supervisor adapter.
...
A toy old generation can start an execution host, hand fenced ownership to a
new foreground process, drain, roll back, and survive a deliberately failed
candidate without losing the host. The development harness also passes its
three trial units, reloads without losing agent/build/evidence state and emits
reproducible secret-free commit attestations.

## Phase 0S — trusted seed and self-hosting foundation

### Work

- Create the private `bext-stack/automonique` staging repository through the audited repository-creation gates; add licence, provenance and governance files matching `LICENSE-POLICY.md` before importing product source.
...
- Split stable development and digest-named candidate state, sockets, process identities, credential audiences, workspaces, artifacts, leases and outboxes.
...
- Build `automonique doctor` for local manifest, foreground process control, optional supervisor adapters, runtime-directory, SQLite, and kernel capability checks, plus a `legacyctl` forwarding entry point.
...
- Start attempt-scoped and session-scoped hosts through the typed direct-process backend; optional adapters may map them to transient units, containers, or remote allocations.
...
- Implement sealed descriptor credential delivery by process class, with optional supervisor credential adapters, so nested tools inherit no provider/control-plane secrets.
...
- Implement `automonique runs/attach/cancel` with legacy forwarding commands.
...
- Implement generation registration, heartbeats, foreground admin-endpoint ownership, reload epochs, readiness, lease primitives and controller fencing; optional adapters translate readiness into native supervisor notifications.
...
- Port Fleet claims, heartbeat, cancellation watch, job logs, terminal reporting, and retry backoff.
...
- Port every unresolved parity-ledger item: client-portal tenant fencing, internal-vs-client publication, companions/knowledge bases, learned targets, Slack live-feed/posting, browser notifications, site digest/oneshot/ops commands, deploy webhook, worker capabilities and audit/reconciliation commands.
...
- Run connector plus `automonique` daemon reload, token/certificate rotation, uninstall/reinstall, revoked consent, rate-limit and expired-interaction drills.
...
- Integrate the ShellDeck desktop application (Rust/GPUI, own repository) over the shared Rust protocol client: remote OIDC transport, multi-session chat, project/Git/artifact/terminal/agent panes and complete settings/management surfaces.
...
- Rewrite the root deploy broker with strict typed input and fd-relative filesystem operations.
...
- Make the Rust foreground runtime the default after soak; supervisor packaging remains optional.
...
| reconciliation scripts and `legacy:audit` | typed `automonique-core` reconcilers plus `automonique audit`; legacy commands forward during migration | 7/8A |
| `dashboard.ts`, `term.ts`, `runs.ts` | `automonique-web`, SDK, `automonique tui`, `automonique` CLI | 8A–8E |
...
| native desktop/PWA/portable clients | ShellDeck (Rust/GPUI) plus TypeScript SDK clients | 8I |
...
| implementation harness, bootstrap and self-hosting | `automonique-bootstrap`, `automonique-lab`, optional clean builder and development SDK | 0/0S |
