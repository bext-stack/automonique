...
## Superseded decisions in this document

Directional decisions below were recorded before the checked-in plan existed.
These four have since been decided differently. The right-hand column is
binding; the text below is retained to show what was considered and rejected.

| Recorded here | Superseded by | Where the binding decision lives |
|---|---|---|
| `automoniquectl` / `automoniqued` as separate binaries | One `automonique` binary with subcommands; privileged brokers stay separate | `requirements/target-architecture.md` § One binary rule |
| Tauri desktop client (decision 89) | ShellDeck, Rust/GPUI, owner-controlled MIT repository | `requirements/client-experience-and-surfaces.md` |
| `GPL-3.0-or-later` | Product `Elastic-2.0`; `sdk/` and `integrations/` `Apache-2.0` | `LICENSE-POLICY.md` |
| "Blocking design decisions are recorded directly in the requirement documents" | Decisions are recorded in the requirement documents *and* gated in the executable plan | `plan/gates.md` |
...
**Reference validation host:** Linux with cgroup v2 and systemd 255 available as
an optional adapter, plus SQLite WAL, Slack Socket Mode, Telegram long polling,
optional public-HTTPS Teams/Discord connectors, and local agent CLIs. The core
foreground runtime does not require systemd.
...
3. [Target architecture](../requirements/target-architecture.md) defines the Rust processes, lifecycle ownership, optional supervisor adapters, component boundaries, and release layout.
...
- ~~Build a Tauri desktop client~~ **superseded — see the table above.** The desktop client is ShellDeck (Rust/GPUI) in its own repository. Optional cross-platform clients still run over the public SDK; UI plugins/widgets remain namespaced and cannot acquire backend authority.
...
- Make the development system functionally self-hosting through explicit SH0–SH6 levels: a stable seed builds an isolated candidate, the candidate rebuilds/reloads itself, stable and any configured clean builder verify it, and only external authority may promote it to production.
...
22. Every implementation ticket is traceable through a reproducible harness run, configured review evidence, required tests and a commit/CI metrics attestation; no skipped/deleted test, unexplained parity difference or threshold regression is hidden by aggregate progress.
23. A clean SH0 bootstrap creates the stable lab; stable builds an immutable candidate; the candidate runs a bounded self-host fixture, rebuilds/reloads itself and falls back under failure; stable and any configured clean builder verify provenance/output; and production promotion remains an external typed action.
...
- Claiming to eliminate trust in the operating system, compiler/toolchain,
  source host, dependency sources, model providers, configured clean builder or
  release authority; self-hosting makes the active roots explicit rather than
