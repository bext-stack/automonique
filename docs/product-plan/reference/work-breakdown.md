...
- **R0-02 Sanitized fixture corpus:** capture Slack, Telegram, Manage, Support and four backend streams. Depends on R0-01. Done when CI can replay without secrets/network.
- **R0-03 Systemd handoff spike:** prove old/new generations, `notify-reload`, `MAINPID`, failure fallback and drain on systemd 255.
- **R0-04 Transient runner-unit spike:** prove a runner survives daemon generation exit and is cancellable as one cgroup.
- **R0-05 Architecture decision:** choose self-handoff or stable launcher from R0-03 results and record the rejected alternative.
...
- **R0-08 Machine-readable parity ledger:** classify every current feature/script/companion as preserve, replace, isolate or retire with owner, fixture and evidence; include Inklura, client publication, audits, live feed, notifications, targets and operational commands.
...
- **R0-20 Author/review/fix trials:** run three representative units with one implementer, two fresh-context adversarial reviewers and one fixer; rerun after harness-policy correction and compare deterministic evidence.
...
- **R0-35 Independent builder/provenance:** distinct workload identity, clean source acquisition, isolated build, authenticated provenance, SBOM and vulnerability/license results.
- **R0-36 Reproducibility comparison:** A1 stable build versus A2 candidate self-build versus A3 independent build, bit-identical target and declared deterministic normalization during transition.
...
- **R1-07 `automoniquectl doctor`:** non-mutating host and release checks plus the `legacyctl` forwarding contract.
...
- **R2-05 systemd transient-unit launcher.**
...
- **R2-28 Dual host lifetimes:** implement attempt-scoped `automonique-run-*` and session-scoped `automonique-agent-*` units with serialized turns, idle TTL and explicit close; discovery recognizes active legacy units during upgrade.
...
- **R2-41 Process-class credentials:** sealed descriptors/systemd credentials, empty nested-tool environments, audience/version revalidation and revocation quarantine.
- **R2-42 Nested tool/MCP/extension host:** separately constrained child units with explicit executable, path, credential, egress and resource grants plus descendant cleanup.
...
- **R3-06 systemd readiness/main-PID handoff or launcher integration.**
...
- **R3-12 Stable admin socket owner:** systemd socket activation or the selected launcher queues/dispatches connections without generation unlink/rebind races.
...
- **R5-03 Manage heartbeat and configuration projection.**
...
- **R7-15 Inklura/client publication parity:** tenant-fenced Support flows and explicit internal-draft versus client-published records.
...
- **R7-18 Reconciliation product:** typed reconcilers and `automoniquectl audit --preview [--full]` / `--apply <plan-id>` (plus legacy alias) for Slack/GitHub/database drift.
...
- **R8D-01 TUI foundation:** `automonique-tui` crate plus `legacy-tui` forwarding entry point, Unix-socket client, reducer, terminal lifecycle guard, responsive layout, keymap and help.
...
- **R8D-10 `automoniquectl` operations:** status, pause, resume, runs, cancel, reload, rollback, audit and TUI launcher over shared contracts, with a tested `legacyctl` forwarding entry point.
...
- **R9-01 Typed Rust Bext broker request parser.**
...
- **R10-02 Runner canary with TypeScript daemon.**
- **R10-03 Rust shadow daemon comparison reports.**
...
- **R12-05 Tauri desktop foundation:** Linux/macOS/Windows packaging, local/remote SDK transport, auth and signed updates.
...
R5 and R6 can proceed in parallel only after R4's identity, journal, receipt, lease and durable-inbox contracts are stable. R8C, R8D and R8F may proceed in parallel only after R8A and the corresponding R8B SDK services are stable. R8F/R8G are optional expansion paths and do not block core R10 cutover when no connector is enabled; Teams and Discord also graduate independently. Rebrand B0–B3 supplies the naming/repository prerequisites for R8B/R8D, while B4 and R10-19 coordinate the one-owner runtime cutover. Required sandbox profiles and provider-control/tool separation block enabled provider cutover; brokered egress, extensions, shell and strong isolation graduate independently. R9 can proceed after the R0 sandbox decision plus shared bounded types, workspace/artifact policy and release tooling exist.
...
- The commit/CI links a reproducible metrics manifest with work/run identity, required checks, independent review result and before/after baselines; missing or incomparable metrics state why.
...
- Implementer, reviewer and fixer evidence remains role-separated; unresolved blocking findings prevent integration.
- A self-hosting ticket states stable/candidate/independent/promoter evidence ownership, candidate credential/data mode, source/build fingerprints and last-known-good rollback path.
