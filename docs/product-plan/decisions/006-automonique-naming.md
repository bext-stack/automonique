...
- The canonical upstream is `bext-stack/automonique`, code releases use the licence boundary established in the repository (product under `Elastic-2.0`; `sdk/` and `integrations/` under `Apache-2.0`, per the checked-in `LICENSE-POLICY.md`), the official product site is `https://automonique.fr`, and Inklura (`https://inklura.fr`) is acknowledged as founding sponsor.
- Sponsor metadata is non-authoritative and never grants repository, tenant, approval or runtime privileges.
- Existing legacy assistant names, `legacy-*`, `LEGACY_*`, `legacy.*` protocol names, Slack commands, database tables and durable IDs are compatibility surfaces, not names to rewrite destructively.
