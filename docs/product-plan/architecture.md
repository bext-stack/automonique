...
├─ automonique-lab.service      durable development control plane
...
├─ automonique.service          replaceable product control-plane generation
├─ automonique-agent@.service   session-scoped provider execution hosts
├─ automonique-run@.service     attempt-scoped execution hosts
└─ connector services           separately credentialed SDK applications
```
...
Product upgrades use generation handoff:
...
Active provider processes are supervised independently of the daemon and emit
sequenced records. Daemon replacement cannot own their lifetime or erase their
session identity.
